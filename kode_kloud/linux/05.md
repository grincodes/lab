...
-  ssh into server 2  ``` ssh username@host ```
-  cd into the home/userdata folder ```cd /home/userdata ```
-  run this command to find  copy files owned by yousuf to /ecommerce directory
    ```
        find . -type f -user yousuf -exec cp --parents {} /ecommerce/ \:
    ```
